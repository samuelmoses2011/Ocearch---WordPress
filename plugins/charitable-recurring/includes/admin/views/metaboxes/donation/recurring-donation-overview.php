<?php 
/**
 * Renders the donation details meta box for the Recurring Donation post type.
 *
...
<div id="charitable-donation-overview-metabox" class="charitable-metabox">
    <div id="donor" class="charitable-media-block">
        ...
        <div class="donor-facts charitable-media-body">
            ...
            <?php 
            /**
             * @hook charitable_recurring_donation_details_donor_facts
             */
            do_action( 'charitable_recurring_donation_details_donor_facts', $donor, $donation );
            ?>
        </div>
    </div>
    <div id="donation-summary">
        <h3 class="donation-number"><?php printf( '%s #%d', 
            __( 'Donation', 'charitable-recurring' ), 
        ...
        <span class="donation-status"><?php printf( '%s: <span class="status">%s</span>', 
            __( 'Status', 'charitable-recurring' ), 
            $donation->get_status( true ) ) ?></span>
    </div>
    <table id="overview">
        <thead>
            <tr>
                <th class="col-campaign-name"><?php _e( 'Campaign', 'charitable-recurring' ) ?></th>
                <th class="col-campaign-donation-amount"><?php _e( 'Total', 'charitable-recurring' ) ?></th>
            </tr>
        </thead>
        <tbody>
        <?php foreach ( $donation->get_campaign_donations() as $campaign_donation ) : ?>
            <tr>
                <td class="campaign-name"><?php echo get_the_title( $campaign_donation['campaign_id'] ) ?></td>
        ...
        <tfoot>
            <tr>
                <th><?php _e( 'Total', 'charitable-recurring' ) ?></th>
                <td><?php echo $donation->get_recurring_donation_amount(true); ?></td>
            </tr>
            <tr>
                <th><?php _e( 'Payment Method', 'charitable-recurring' ) ?></th>
                <td><?php echo $donation->get_gateway_label() ?></td>
            </tr>
            <tr>
                <th><?php _e( 'Change Status', 'charitable-recurring' ) ?></th>
                <td>
                    <select id="change-donation-status" name="post_status">
                    <?php foreach ( charitable_recurring_get_valid_donation_statuses() as $status => $label ) : ?>
            ...
            <tr class="hide-if-js">
                <td colspan="2">
                    <input type="submit" name="update" class="button button-primary" value="<?php _e( 'Update Status', 'charitable-recurring' ) ?>" />
                </td>
            </tr>
        </tfoot>
    </table>
